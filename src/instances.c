...
void instances_management() {
    ...
    if (poll_result <= 0) {
        // TODO : Really weird shit is happening here.
        // /home/http/dev/github/DStudio/src/instances.c 114: "poll_result <=0 : No such file or directory"
...
void dstudio_new_client_instance(
    ...
    if (processes_count > 1) {
        ...
        fclose(new_instance);
        g_instances.data = dstudio_alloc(
            sizeof(InstanceContext),
            DSTUDIO_FAILURE_IS_FATAL
        );
        g_instances.count += 1;
...
void update_ui_instances_list() {
    ...
    if (g_ui_instances.update_request) {
        g_ui_instances.source_data = g_instances.data;
