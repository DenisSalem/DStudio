...
int main(int argc, char ** argv) {
    ...
    DSTUDIO_SET_AREA(0, 20.0, 85.0, 147.0, 212.0) 
    DSTUDIO_SET_UI_CALLBACK(0, update_knob, 0, sample_knobs_p, DSTUDIO_KNOB_TYPE);

    // Knobs SAMPLE : END
    init_knob(sample_knobs_p, 1, -0.7075, 0.25);
    DSTUDIO_SET_AREA(1, 84.0, 149.0, 147.0, 212.0) 
    DSTUDIO_SET_UI_CALLBACK(1, update_knob, 1, sample_knobs_p, DSTUDIO_KNOB_TYPE);

    // Knobs SAMPLE : GRAIN SIZE
    init_knob(sample_knobs_p, 2, -0.5475, 0.25);
    DSTUDIO_SET_AREA(2, 148.0, 213.0, 147.0, 212.0) 
    DSTUDIO_SET_UI_CALLBACK(2, update_knob, 2, sample_knobs_p, DSTUDIO_KNOB_TYPE);
    
    // Knobs SAMPLE : TRANSIENT THRESOLD
    init_knob(sample_knobs_p, 3, -0.3875, 0.25);
    DSTUDIO_SET_AREA(3, 212.0, 277.0, 147.0, 212.0) 
    DSTUDIO_SET_UI_CALLBACK(3, update_knob, 3, sample_knobs_p, DSTUDIO_KNOB_TYPE);

    // Knobs SAMPLE : LOOP DURATION
    init_knob(sample_knobs_p, 4, -0.8675, -.15);
    DSTUDIO_SET_AREA(4, 20.0, 85.0, 243.0, 308.0) 
    DSTUDIO_SET_UI_CALLBACK(4, update_knob, 4, sample_knobs_p, DSTUDIO_KNOB_TYPE);

    // Knobs SAMPLE : CLOUD SIZE
    init_knob(sample_knobs_p, 5, -0.7075, -.15);
    DSTUDIO_SET_AREA(5, 84.0, 149.0, 243.0, 308.0) 
    DSTUDIO_SET_UI_CALLBACK(5, update_knob, 5, sample_knobs_p, DSTUDIO_KNOB_TYPE);
    
    // Knobs SAMPLE : DISTRIBUTION BALANCE
    init_knob(sample_knobs_p, 6, -0.5475, -.15);
    DSTUDIO_SET_AREA(6, 148.0, 213.0, 243.0, 308.0) 
    DSTUDIO_SET_UI_CALLBACK(6, update_knob, 6, sample_knobs_p, DSTUDIO_KNOB_TYPE);

    // Knobs SAMPLE : SMOOTH TRANSITION
    init_knob(sample_knobs_p, 7, -0.3875, -.15);
